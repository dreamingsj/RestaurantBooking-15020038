...
class BookingSchedulerTest(unittest.TestCase):
    ...
    def test_이메일이_없는_경우에는_이메일_미발송(self):
        ...
        self.mail_sender.send_mail.asser_not_called()
    ...
    def test_이메일이_있는_경우에는_이메일_발송(self):
        ...
        self.mail_sender.send_mail.asser_called()

    @patch.object(BookingScheduler, 'get_now', return_value=datetime.strptime("2021/03/28 07:00", "%Y/%m/%d %H:%M"))
    ...
    @patch.object(BookingScheduler, 'get_now', return_value=datetime.strptime("2021/06/03 07:00", "%Y/%m/%d %H:%M"))
